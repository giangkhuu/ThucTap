            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-light flex-lg-nowrap justify-content-center justify-content-lg-start">
                    ...
                    <li class="breadcrumb-item text-nowrap">
                        <a href="{{ route('user.giohang') }}">Giỏ hàng</a>
                    </li>
    ...
    <div class="row">
        <section class="col-lg-8">
            <div class="steps steps-light pt-2 pb-3 mb-5">
                <a class="step-item active" href="{{ route('user.giohang') }}">
            ...
            <form method="post" action="{{ route('user.dathang') }}" class="needs-validation" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control @error('dienthoaigiaohang') is-invalid @enderror" type="text" id="dienthoaigiaohang" name="dienthoaigiaohang" value="{{ Auth::user()->sdt ?? '' }}" required />
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control @error('diachigiaohang') is-invalid @enderror" type="text" id="diachigiaohang" name="diachigiaohang" value="{{ Auth::user()->diachi ?? '' }}" required />
            ...
            <div class="d-none d-lg-flex pt-4 mt-3">
                <div class="w-50 pe-3">
                    <a class="btn btn-secondary d-block w-100" href="{{ route('user.giohang') }}">
            ...
            <div class="bg-white rounded-3 shadow-lg p-4 ms-lg-auto">
                <div class="py-2 px-xl-2">
                    <div class="widget mb-3">
                        <h2 class="widget-title text-center">Thuốc đã đặt</h2>
                        @foreach($giohang as $value)
                        <div class="d-flex align-items-center pb-2 border-bottom">
                            <a class="d-block flex-shrink-0" href="#">
                                <img src="{{ env('APP_URL') . '/storage/app/' . $value->thuoc->hinhanh }}" width="64" />
                            </a>

                            <div class="ps-2">
                                <h6 class="widget-product-title">{{ $value->thuoc->tenthuoc }}</h6>
                                <div class="widget-product-meta">
                                    <span class="text-accent me-2">{{ number_format($value->dongiaban, 0, ',', '.') }}<small>đ</small></span>
                                    <span class="text-muted">x {{ $value->soluong }}</span>
                                </div>
                    ...
                    <ul class="list-unstyled fs-sm pb-2 border-bottom">
                        <li class="d-flex justify-content-between align-items-center">
                            <span class="me-2">Tổng tiền sản phẩm:</span><span class="text-end">{{ number_format($tongtien, 0, ',', '.') }}<small>đ</small></span>
                        </li>
                        <li class="d-flex justify-content-between align-items-center">
                            <span class="me-2">Phí vận chuyển:</span><span class="text-end"><!-- 30 ngàn --></span>
                        </li>
                        <li class="d-flex justify-content-between align-items-center">
                            <span class="me-2">Thuế GTGT:</span><span class="text-end"><!-- Thuế 10% --></span>
                        </li>
                    </ul>
                    <h3 class="fw-normal text-center my-4">{{ number_format($tongtien, 0, ',', '.') }}<!-- Tổng giá trị phải thanh toán --><small>đ</small></h3>
                    @if(session('error'))
